...
class PreferenceUtils {
  ...
  static int? getCoachInt(String key, [int? defValue]) {
  ...
  static Future<bool> setCoachInt(String key, int value) async {
    var prefs = await _instance;
    return prefs?.setInt(key, value) ?? Future.value(false);
  }

  static Future<bool> setRemove(String key, int value) async {
    var prefs = await _instance;
    return prefs?.setInt(key, value) ?? Future.value(false);
  }
  ...
  static Future<bool?> reloade(String key)async{
    var prefs = await _instance;
    prefs!.reload();
  }
